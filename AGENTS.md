...
## Skills & specialized files

When creating specialized files (Word documents, PowerPoint presentations, Excel spreadsheets, PDFs):

- **Always read the appropriate skill documentation first** using the `view` tool before creating any specialized files.
- Skills are located in `/mnt/skills/` with subdirectories: `public/`, `private/`, `user/`, `examples/`.
- Skills contain proven best practices refined through iteration and must be followed for professional output.
- Common public skills:
  - `/mnt/skills/public/docx/SKILL.md` - Word documents (.docx)
  - `/mnt/skills/public/pptx/SKILL.md` - PowerPoint presentations (.pptx)
  - `/mnt/skills/public/xlsx/SKILL.md` - Excel spreadsheets (.xlsx)
  - `/mnt/skills/public/pdf/SKILL.md` - PDF files
- User-provided skills (`/mnt/skills/user/`) take precedence when relevant to the task.
- Multiple skills may be required for complex deliverables.
- Read skills sequentially if multiple are needed.
- After reading relevant skills, follow their instructions precisely.

**Workflow for specialized files:**

1. Identify which skill(s) are relevant
2. Use `view` tool to read the skill documentation
3. Follow the skill's guidelines when creating the file
4. Place final output in `/mnt/user-data/outputs/`
